...
def apply_logging_config():
    import cata_log.logging  # noqa: PLC0415 # toplevel import messes up tmp_paths

    logging.config.dictConfig(cata_log.logging.CATA_LOG_LOGGING_CONFIG)
    logging.config.dictConfig(cata_log.logging.UVICORN_LOGGING_CONFIG)
    yield
    logging.shutdown()
...
def test_loggers(capsys, faker, apply_logging_config):
    ...
    logging.getLogger("cata_log").error(fake_message)

    assert fake_message in capsys.readouterr().err
    assert fake_message in (get_settings().logs_path / "cata-log.log").read_text()
